                              ...
                              <div key={stop} className="mb-4">
                                  ...
                                  <div className="space-y-2 border-l-2 border-slate-100 pl-3 ml-2">
                                      {stopStudents.map(student => {
                                          ...
                                          const owed = student.total_fee - student.paid;
                                          return (
                                              <div key={student.id} className={`p-3 rounded-xl border ${selectedStudentId === student.id ? 'border-indigo-500 bg-indigo-50 shadow-md' : 'border-slate-200 bg-white'} flex flex-col gap-2`}>
                                                  <div className="flex justify-between items-start">
                                                      <div>
                                                          <div className="flex items-center">
                                                              <h3 className="font-bold text-sm text-slate-800 mr-2">{student.name}</h3>
                                                              <button onClick={() => openEditModal(student)} className="text-indigo-600 bg-indigo-100 px-2 py-0.5 rounded text-[10px] font-bold">Edit</button>
                                                          </div>
                                                          <div className="flex gap-1 mt-1">
                                                              <span className="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded border font-medium">{student.grade}</span>
                                                              <span className={`text-[10px] px-1.5 py-0.5 rounded border font-bold ${getPlanColor(student.plan)}`}>{student.plan}</span>
                                                          </div>
                                                      </div>
                                                      <div className="flex flex-col items-end gap-1">
                                                          <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full border ${status.color}`}>
                                                              {status.label}
                                                          </span>
                                                          <button onClick={() => shareReceiptWA(student)} className="text-green-600 bg-green-50 p-1.5 rounded-full border border-green-200 shadow-sm active:scale-95 transition-transform" title="WhatsApp Receipt">
                                                  ...
                                                  <div className="flex justify-between items-center mt-1 border-t pt-2 border-slate-100">
                                                      <div className="text-xs text-slate-600">
                                                          Paid: <b className="text-slate-800">GH₵{student.paid}</b> / {student.total_fee}
                                                      </div>
                                                      {student.paid < student.total_fee && (
                                                          <div className="flex gap-2">
                                                              <button onClick={() => handleQuickPay(student)} className="bg-amber-100 text-amber-700 text-[10px] font-bold px-2 py-1 rounded shadow-sm active:scale-95 transition-transform">Exact GH₵{owed}</button>
                                                              <button onClick={() => handleAddPaymentClick(student)} className="bg-indigo-600 text-white text-[10px] font-bold px-3 py-1 rounded shadow-sm active:scale-95 transition-transform">Pay</button>
                                                          </div>
                                                      )}
                                                  </div>
                                              </div>
